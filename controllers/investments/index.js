...
exports.transferInvestmentController = async (req, res) => {
    let investmentObj  = {...req.body};
    investmentObj['userId'] = req.userId;
    try{
        //if(req.userRole === "subscriber"){
            const investmentService = Container.get("InvestmentService")
            let response = await investmentService.TransferInvestment(investmentObj, req.userId)
            if (response) return res.status(200).json({status: "ok", data: "ok"});
            return res.status(201).json({status: "Fail to add new investment into portfolio"})
        //}else{
          //  return res.status(400).json({status: "Fail to add new investment into portfolio"})
        //}
...
exports.getAllInvestment = async (req, res) => {
    try{
        const investmentService = Container.get("InvestmentService")
        let response = await investmentService.GetListedProps(req.userId)
        if (response) return res.status(200).json({status: "ok", data : response});
        return res.status(201).json({status: "no investments founded"})
    }catch(e){
        logger.error('🔥 error: '+ e);
        //return next(e);
        return res.status(400).json({
            error: "Failed to get investments with Local API",
        });
    }
}
...
exports.populateDB = async (req, res) => {
    try{
        const investmentService = Container.get("InvestmentService")
        let response = await investmentService.PopulateDB()
        if (response) return res.status(200).json({status: "ok"});
        return res.status(201).json({status: "already deleted"})
    }catch(e){
        logger.error('🔥 error: '+ e);
